...
describe('Dashboard page', () => {
  ...
  it.each(['admin', 'editor'])('redirects an authenticated %s user to login', async (role) => {
    getCurrentUserMock.mockResolvedValue({ id: 1, email: `${role}@example.com`, role })
    const { default: Dashboard } = await import('./page')
  ...
  it('renders a personal welcome and next-release placeholder for signed-in users', async () => {
    ...
    expect(html).toContain(
      'Document upload and PT PMA setup tracking land in the next portal release.',
    )

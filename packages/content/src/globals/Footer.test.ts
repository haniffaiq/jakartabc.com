...
describe('Footer global', () => {
  it('has address/email/licenses/legal links fields and the revalidate hook', () => {
    expect(Footer.slug).toBe('footer')

    const fields = Footer.fields as Array<{ name?: string; localized?: boolean; type?: string; fields?: Array<{ name?: string; validate?: unknown }> }>
    const names = fields.map((field) => field.name)
    expect(names).toEqual(expect.arrayContaining(['address', 'email', 'licenses', 'legalLinks', 'notice']))
    expect(fields.find((field) => field.name === 'address')).toMatchObject({ type: 'array', localized: true })
    expect(fields.find((field) => field.name === 'licenses')).toMatchObject({ type: 'array', localized: true })
    expect(fields.find((field) => field.name === 'notice')).toMatchObject({ type: 'richText', localized: true })
